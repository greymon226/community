...
| AI 原生证据 | Kiro Spec 三段式 + 37 条 PBT + 4 个 Hooks + MCP Server + 8 节点协作实录 |
...
**差异化**：大多数参赛作品是"项目用 AI"，我们还做到了"AI 用项目"。
...
| 价值点 | 说明 |
| --- | --- |
| 需求可追溯 | 84 AC → 37 Property → 37 PBT，全链路闭环 |
...
| 测试方法 | 手写单元测试 | 37 条 PBT ≥ 3700 次迭代 |
| AI 原生证据 | 截图/文字描述 | 8 节点协作实录 + Hook 配置 |
| 运行时 AI 守护 | 无 | 4 个 Kiro Hooks |
...
**步骤 7：MCP Server 演示（45s）**
- 在 Kiro IDE 对话框输入："搜索社区里关于 useEffect 闭包的帖子"
- Kiro 自动调用 `search_posts` 工具
- 展示返回的帖子列表
- 输入："公司内部 Docker 部署最佳实践"
- Kiro 调用 `ask_community`，基于站内知识回答
...
A：
- 让外部 AI 助手（Kiro / Claude Desktop）可以直接"问站内"
- 实现"双向 AI 原生"：项目用 AI + AI 用项目
- 4 个工具：搜索/获取/问答/推荐
- 安全：stdio 模式仅本地访问，`ask_community` 需人工确认
