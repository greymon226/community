...
| PBT 测试 | 29 个可执行 PBT 文件，覆盖 37 条 Property，全部通过 |
| 测试总数 | 147 个断言，0 失败 |
...
| `.github/workflows/test.yml` | CI：push/PR 自动跑 unit + property 测试 |
...
4 个 Hook 把 AI 从"工具"升级为"流程一部分"（已默认启用，文件位于 `.kiro/hooks/*.kiro.hook`）：
...
ai-collaboration-log.md (8 节点实录)
  → requirements.md (27 需求 / 84 AC)
    → design.md (37 Properties)
      → tests/property/P*.test.js (29 个可执行 PBT 文件)
        → .kiro/hooks/ (4 个运行时守护，已启用)
...
| CORS 来源限制 | — | 生产配置 | `CORS_ORIGINS` 白名单，禁止任意跨域 |
| 并发写安全 | P16 | P16 测试 | `increment`/`decrement` + `reload()` 防计数器漂移 |
...
| 文件 | 测试场景 |
| --- | --- |
| `auth_basic.e2e.js` | 登录 → 分类 → 发帖 → 点赞收藏 → 评论 → 加精 → 搜索 → stats → 通知 |
| `settings_toggle.e2e.js` | AI 审核开关：开启 vs 关闭时 `aiAuditStatus` 表现 |
| `ai_audit.e2e.js` | DeepSeek 协议层：mock 模型返回 + 失败降级 |
| `post_block.e2e.js` | AI blocked → HTTP 400 + code=4002；草稿不走 AI |
| `ai_explain.e2e.js` | AI 解读 → 缓存命中 → 开关关闭 4003 |
...
```bash
# 运行全部 Property 测试（29 个可执行 PBT 文件）
cd backend && npm run test:property

# 运行单个 Property（如 P37 Prompt Injection）
cd backend && node --test tests/property/P37-prompt-injection-detection.test.js
