...
async function run() {
  ...
  if (sampleAuthor) {
    ...
    if (cnt === 0) {
      ...
      const tAI = await Tag.findOne({ where: { name: 'AI' } });
      if (tAI) await PostTag.create({ postId: post1.id, tagId: tAI.id });
      ...
      const tVue = await Tag.findOne({ where: { name: 'Vue' } });
      if (tVue) await PostTag.create({ postId: post2.id, tagId: tVue.id });
      ...
      const tReact = await Tag.findOne({ where: { name: 'React' } });
      if (tReact) await PostTag.create({ postId: post3.id, tagId: tReact.id });
      const tPerf = await Tag.findOne({ where: { name: '性能优化' } });
      if (tPerf) await PostTag.create({ postId: post3.id, tagId: tPerf.id });
      ...
      const tNode = await Tag.findOne({ where: { name: 'Node.js' } });
      if (tNode) await PostTag.create({ postId: post4.id, tagId: tNode.id });
      if (tPerf) await PostTag.create({ postId: post4.id, tagId: tPerf.id });
      ...
      const tMS = await Tag.findOne({ where: { name: '微服务' } });
      if (tMS) await PostTag.create({ postId: post5.id, tagId: tMS.id });
      ...
      const tCI = await Tag.findOne({ where: { name: 'CI/CD' } });
      if (tCI) await PostTag.create({ postId: post8.id, tagId: tCI.id });
      ...
      if (tAI) await PostTag.create({ postId: post9.id, tagId: tAI.id });
      ...
      const tInterview = await Tag.findOne({ where: { name: '面试' } });
      if (tInterview) await PostTag.create({ postId: post10.id, tagId: tInterview.id });
